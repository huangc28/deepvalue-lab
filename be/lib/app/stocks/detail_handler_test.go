...
func TestDetailHandlerLocaleSelection(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "locale=zh-TW falls back to zh-TW summary",
			url:        "/v1/stocks/TSM?locale=zh-TW",
			...
			wantBody:   `{"summary":"zh-summary"}`,
			wantKey:    "",
			wantStatus: http.StatusOK,
		},
		{
			name:       "locale=zh-TW falls back to en summary when no locale data exists",
			url:        "/v1/stocks/TSM?locale=zh-TW",
			row:        turso_models.PublishedStockDetail{Ticker: "TSM", SummaryJson: `{"summary":"en-summary"}`},
			dataByKey:  map[string][]byte{},
			wantBody:   `{"summary":"en-summary"}`,
			wantKey:    "",
			wantStatus: http.StatusOK,
		},
		{
