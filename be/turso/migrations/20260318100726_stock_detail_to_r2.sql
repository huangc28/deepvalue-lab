...
INSERT INTO published_stock_details_new (ticker, report_id, summary_json, published_at_ms, updated_at_ms)
SELECT ticker, report_id, stock_detail, published_at_ms, updated_at_ms
...
BEGIN
  UPDATE published_stock_details SET updated_at_ms = (unixepoch('now') * 1000) WHERE ticker = NEW.ticker;
END;
-- +goose StatementEnd
...
INSERT INTO published_stock_details_old (ticker, report_id, stock_detail, published_at_ms, updated_at_ms)
SELECT ticker, report_id, summary_json, published_at_ms, updated_at_ms
FROM published_stock_details;
